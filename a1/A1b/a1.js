...
function drawPolygon(ctx, poly)
{
  ...
  for (i = 0; i < poly.length; i++) { 
    ...
    drawLineSegment(ctx, current[0], current[1], next[0], next[1]);
}
...
function rotationAboutPoint(angle, joint)
{
 /* var T = [
  ...
  T4 = composeTransforms(T1, T2);
  T5 = composeTransforms(T4, T3);
...
function drawPenguin(ctx)
{
  ...
  var leg1_poly = [[-15, -53], [-20, 52], [ 20,  52], [ 12, -53]];
  var foot1_poly = [[54, -11], [58, 11], [-48,  11], [-58, -11]];
  ...
  //draw upper_beak
  var upper_beak_origin = [-80,0];
  var upper_beak_T = translateByOffset([upper_beak_origin[0], upper_beak_origin[1] - mouth_gap]);
  ...
  //draw lower_beak
  var lower_beak_origin = [-80,25];

  var lower_beak_T = translateByOffset([lower_beak_origin[0], lower_beak_origin[1] + mouth_gap]);
  ...
  //draw arm
  var arm_offset = [0, -20]; // The location of the arm relative to the body
  var arm_joint_offset = [0, -20]; // The location of the arm joint relative to the head center
  ...
  //draw the leg
  var leg_offset = [-70, 170]; // The location of the leg relative to the body
  var hip_joint_offset = [0, -20]; // The location of the leg joint relative to the head center
  ...
  //Draw the hip
  // Now draw the hip joint
  hip_joint = transformPoint(hip_joint_offset, leg_T);
  hip_joint = transformPoint(hip_joint, torso_T);
  drawCircle(ctx, hip_joint[0], hip_joint[1], hip_joint_r);

  var foot_offset = [-40,40]; // The location of the hip relative to the body
  var ankle_joint_offset = [35, 0]; // The location of the hip joint relative to the head center
  ...
  // That is, if the hip moves, then the foot will move with it.
  ...
  // Now draw the ankle joint
  ankle_joint = transformPoint(ankle_joint_offset, foot_T);
  ankle_joint = transformPoint(ankle_joint, leg_T);
  ankle_joint = transformPoint(ankle_joint, torso_T);
  drawCircle(ctx, ankle_joint[0], ankle_joint[1], ankle_joint_r);



  var leg1_offset = [60, 180]; // The location of the hip relative to the body
  var hip1_joint_offset = [0, -20]; // The location of the hip joint relative to the head center
  var leg1_T = composeTransforms(
    rotationAboutPoint(hip_angles[1], hip1_joint_offset),
    translateByOffset(leg1_offset)
  );
 
  //the other leg, hip, ankle
  leg1_poly = transformPolygon(leg1_poly, leg1_T);
  leg1_poly = transformPolygon(leg1_poly, torso_T);
  drawPolygon(ctx, leg1_poly);

  // Now draw the head joint
  hip1_joint = transformPoint(hip1_joint_offset, leg1_T);
  hip1_joint = transformPoint(hip1_joint, torso_T);
  drawCircle(ctx, hip1_joint[0], hip1_joint[1], hip_joint_r);

  var foot1_offset = [-40,40]; 
  var ankle1_joint_offset = [35, 0]; 
  var foot1_T = composeTransforms(
    rotationAboutPoint(ankle_angles[1], ankle1_joint_offset),
  ...
  ankle1_joint = transformPoint(ankle1_joint_offset, foot1_T);
  ankle1_joint = transformPoint(ankle1_joint, leg1_T);
  ankle1_joint = transformPoint(ankle1_joint, torso_T);
  drawCircle(ctx, ankle1_joint[0], ankle1_joint[1], ankle_joint_r);
